<script>
    ...
    const onExport = () => {
        ...
        if (clothesData[state.componentId].type !== "tops") {
            state.export = `INSERT INTO clothes_${state.gender === 'men' ? 'male' : 'female'}_${clothesData[state.componentId].type}(id, cvariation, textures, category, can_buy, price) VALUES ('${state["drawable"]}', '${state["drawable"] - MaxClothesComponent[state["gender"]][clothesData[state.componentId].type]}', '${JSON.stringify(textures)}', '', '0', '0');`;
        } else {
    ...
    <div class="box-content clothesEditor">
        <div class="box-ch">
            <div class="box-max" on:mouseenter={() => executeClient("client.camera.toggled", false)} on:mouseleave={() => executeClient("client.camera.toggled", true)}>
                ...
                <div class="box-list" style="max-height: 100%">
                    <div class="switch">
                        <div id="gendermale" class="btn" on:click={() => setGender('men')}>
                            <span class={state.gender === 'men' ? "active" : ""}/>
                            {translateText('player', 'Male')}
                        </div>
                        <div id="genderfemale" class="btn" on:click={() => setGender('women')}>
                            <span class={state.gender === 'women' ? "active" : ""}/>
                            {translateText('player', 'Female')}
                        </div>
                    </div>

                    <div class="label">{translateText('player', 'Component')}:</div>
                    ...
                    <div class="module-list">
                        ...
                        <input
                            bind:value={state.drawable}
                            on:input={event => {
                                if (event.target.value.replace(/\D/,'')) {
                                    executeClient(
                                        'client.clothesEditor.update',
                                        clothesData[state["componentId"]].id,
                                        clothesData[state["componentId"]].props,
                    ...
                    {#if clothesData[state["componentId"]].type === "tops"}
                        ...
                        <div class="module-list">
                            ...
                            <input
                                value={TorsosData[state["gender"]][state.torsos]}
                                on:input={event => {
                                    ...
                                    if (valueTorsos && TorsosData[state["gender"]][valueTorsos] !== undefined) {
                                        ...
                                        executeClient(
                                            ...
                                            clothesDataToName["torsos"].id,
                                            clothesDataToName["torsos"].props,
                                            TorsosData[state["gender"]][valueTorsos],
                                            0
                                        );
                                    }
                                }}
            ...
            <div class="bottom-btn"> 
                <div class="btn green" on:click={onExport}>{translateText('player', 'Export')}</div>
                <div class="btn red" on:click={onExit}>
                    {state.export == false ? translateText('player', 'Exit') : translateText('player', 'Back')}
                </div>
            </div>
        </div>
        ...
        <div class="box-info-button">
            <div class="bottom-btn">
                ...
                <div class="text">{translateText('player', 'LMB - inspect')}</div>
            </div>
        </div>
    </div>
</div>
